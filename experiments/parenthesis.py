...
from csp import (
    ...
    generate_parenthesis_data, create_dataloaders,
)
from utils import (
...
def main():
    ...
    for seed,idx in enumerate(SEEDS):
        ...
        for label, Model, Param in reversed(zipped):
            ...
            # ---------- Train or load ----------
            if not eval_:
                X, y = generate_parenthesis_data(train_samples, min_len=2,max_len=train_len)
            ...
            for len_ in eval_lens:
                X, y = generate_parenthesis_data(test_samples,min_len=2, max_len=len_)
